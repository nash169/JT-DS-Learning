...
arm1 = SerialLink(L, 'name','arm1', 'base', transl([0, 0, 0]));
...
robotplant = RobotPlant(arm1, 'end_trans');
...
traj1 = [linspace(-pi/2 -.4, - pi/2 + .6, 50) linspace( - pi/2 + .6, -pi/2 + .5, 50);
...
[Data, index] = preprocess_demos_jtds(robot(1).arm1, demos, dt, tol_cutting);
%% Model Learning

% Options for the JT-DS learner.
options.latent_mapping_type = 'PCA';
options.autoencoder_num_dims = 1;
...
% load Data.mat;
[Priors, Mu, Sigma, As, latent_mapping] = JTDS_Solver(Data,robot(1).arm1,options);
...
%% Visualizing
robotVisualize = PlayTraj(robot, 'learned', robotVisualize);
